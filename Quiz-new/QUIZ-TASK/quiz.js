...
/* ---------------- Auth + User Stats (localStorage) ---------------- */
var USERS_KEY = "quizverse_users_v1";
var SESSION_KEY = "quizverse_session_v1";
var STATS_KEY = "quizverse_user_stats_v1";
...
function updateUserStats(email, entry){
  var all = readStatsAll();
  var s = all[email];
  if (!s){
    s = { attempted: 0, totalPoints: 0, bestScore: 0, bestAccuracy: 0, history: [] };
  }
  s.attempted += 1;
  s.totalPoints += entry.pointsEarned;

  if (entry.score > s.bestScore) s.bestScore = entry.score;
  if (entry.accuracy > s.bestAccuracy) s.bestAccuracy = entry.accuracy;
...
var sessionUser = ensureLoggedIn();
...
function renderUserBar(){
  ...
  if (userNameEl) userNameEl.textContent = sessionUser.name || sessionUser.username || "User";
  if (userEmailEl) userEmailEl.textContent = sessionUser.email || "";
...
function renderUserStatsBox(){
  if (!sessionUser || !userStatsBox) return;

  clearEl(userStatsBox);

  var stats = getUserStats(sessionUser.email);

  var title = document.createElement("div");
  title.className = "userStatsTitle";
  title.textContent = "Your Stats";
  userStatsBox.appendChild(title);

  var grid = document.createElement("div");
  grid.className = "userStatsGrid";

  function addStat(label, value){
    var card = document.createElement("div");
    card.className = "userStat";

    var l = document.createElement("div");
    l.className = "userStatLabel";
    l.textContent = label;

    var v = document.createElement("div");
    v.className = "userStatValue";
  ...
  addStat("Tests attempted", stats.attempted);
  addStat("Total points", stats.totalPoints);
  addStat("Best score", stats.bestScore);
  addStat("Best accuracy", stats.bestAccuracy + "%");

  userStatsBox.appendChild(grid);
}
...
var userStatsBox = $("#userStatsBox");
var pointsEarnedText = $("#pointsEarnedText");
var totalPointsText = $("#totalPointsText");
...
function submitExam(auto){
  ...
  // points + user stats
  var pointsEarned = correct * 10; // simple scoring: 10 points per correct answer
  if (pointsEarnedText) pointsEarnedText.textContent = String(pointsEarned);

  var updated = null;
  if (sessionUser && sessionUser.email){
    updated = updateUserStats(sessionUser.email, {
      at: Date.now(),
  ...
  if (updated && totalPointsText) totalPointsText.textContent = String(updated.totalPoints);
  if (!updated && totalPointsText && sessionUser && sessionUser.email){
    totalPointsText.textContent = String(getUserStats(sessionUser.email).totalPoints);
  }

  // refresh start-screen stats too
  renderUserStatsBox();
...
restartBtn.addEventListener("click", function(){
  ...
  if (pointsEarnedText) pointsEarnedText.textContent = "--";
  if (totalPointsText && sessionUser && sessionUser.email){
    totalPointsText.textContent = String(getUserStats(sessionUser.email).totalPoints);
  }
